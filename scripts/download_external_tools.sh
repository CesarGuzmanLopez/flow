...
TEST_URL="https://github.com/CesarGuzmanLopez/test-app-epa/releases/download/master/WebTEST.jar"

# Target filenames
AMBIT_SYNTH_JAR="$AMBIT_DIR/SyntheticAccessibilityCli.jar"
TEST_JAR="$TEST_DIR/WebTEST.jar"
...
echo "=== EPA WebTEST ==="
echo "Target: $TEST_JAR"
fetch "$TEST_URL" "$TEST_JAR"
validate_jar "$TEST_JAR"
echo "✅ EPA WebTEST ready"

echo
echo "External tools ready under: $TOOLS_DIR"
ls -lh "$AMBIT_SYNTH_JAR" "$TEST_JAR" 2>/dev/null || true
